...
//     int gpr_0, int gpr_1, int gpr_2, int gpr_3, int gpr_4, int gpr_5, int
//     gpr_6, int gpr_7, int gpr_8, int gpr_9, int gpr_10, int gpr_11, int
//     gpr_12, int gpr_13, int gpr_14, int gpr_15, int gpr_16, int gpr_17, int
//     gpr_18, int gpr_19, int gpr_20, int gpr_21, int gpr_22, int gpr_23, int
//     gpr_24, int gpr_25, int gpr_26, int gpr_27, int gpr_28, int gpr_29, int
//     gpr_30, int gpr_31) {
//   dut_ptr->gpr[0] = gpr_0;
//   dut_ptr->gpr[1] = gpr_1;
//   dut_ptr->gpr[2] = gpr_2;
...
void difftest_count_phySign() {
  ...
  __ASSERT_SIM__(count == 32, "phy alloc total is not 32 but {}", count);
}
...
extern "C" void v_difftest_PhyRegInFreeList(char io_flHead, char io_flTail,
  ...
  for (int i = io_flTail; i < io_flHead; i = (i + 1) % 64) {
    auto index = i & 0x1f;
    phyUse[(unsigned)io_fl[index]] = true;
  }
}
extern "C" void v_difftest_PhyRegInROB(char io_robHead, char io_robTail,
  ...
  if (!io_isRecover)
    for (int i = io_robTail; i < io_robHead; i = (i + 1) % 64) {
      if (io_rob[i & 0x1f])
        phyUse[(unsigned)io_rob[i & 0x1f]] = true;
    }
  else
    for (int i = io_flrTail; i < io_flrHead; i = (i + 1) % 64) {
      if (io_flr[i & 0x1f])
        phyUse[(unsigned)io_flr[i & 0x1f]] = true;
    }
}