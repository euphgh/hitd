...
bool mips32_CPU_state::ref_exec_once(bool mycpu_int) { /*{{{*/
  ...
  if (running == false)
    nemu_ref_end_statistics(nemu_state.state, log_pt);
  else
    trace_and_difftest(&inst_state);
  return running;
} /*}}}*/

void CPU_state::ref_set_hilo(word_t _hi, word_t _lo) {
  arch_state.hi = _hi;
  arch_state.lo = _lo;
}

void CPU_state::ref_set_gpr(word_t data, uint8_t wnum) {
  arch_state.gpr[wnum] = data;
}
