...
#define EXPT(...) try { __VA_ARGS__; } catch (int e) {}
  ...
  INSTPAT("010001 01110   11111   00000   00011  100000", ri_ft  , N, EXPT(isa_raise_intr(MUXDEF(CONFIG_MIPS_RLS1,EC_CpU,EC_RI), inst_state.pc)));// only func test need RI else is 
  ...
  INSTPAT("11???? ?????   ?????   ?????   ?????  ??????", ri_11  , N, EXPT(isa_raise_intr(EC_RI, inst_state.pc)));
