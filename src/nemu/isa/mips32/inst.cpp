...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("101001 ?????   ?????   ????? ????? ??????", sh     , I, word_t addr = src1 + imm; if (data_addr_ok(addr, 2, AdES)) Mw(addr, 2, R(rd)));
  ...
  INSTPAT("010000 00000   ?????   ????? 00000 000???", mfc0   , U, Rw(rd, cp0_read(&cp0, (imm | imm>>8)))); 
  INSTPAT("010000 00100   ?????   ????? 00000 000???", mtc0   , U, cp0_write(&cp0, (imm | imm>>8), R(rd)));
