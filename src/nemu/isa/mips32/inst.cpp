...
#define EXPT(...) try { __VA_ARGS__; } catch (int e) {}
  ...
  INSTPAT("011100 ?????   ?????   00000 00000 000001", maddu  , R, inst_maddu(src1, src2));
  INSTPAT("011100 ?????   ?????   00000 00000 000100", msub   , R, inst_msub(src1, src2));
  // INSTPAT("011100 ?????   ?????   00000 00000 000000", madd   , N, fmt::print(ANSI_FMT("MADD\n", ANSI_FG_BLUE));EXPT(isa_raise_intr(EC_RI, inst_state.pc)));
  // INSTPAT("011100 ?????   ?????   00000 00000 000001", maddu  , N, fmt::print(ANSI_FMT("MADDU\n", ANSI_FG_BLUE));EXPT(isa_raise_intr(EC_RI, inst_state.pc)));
  // INSTPAT("011100 ?????   ?????   00000 00000 000100", msub   , N, fmt::print(ANSI_FMT("MSUB\n", ANSI_FG_BLUE));EXPT(isa_raise_intr(EC_RI, inst_state.pc)));
  ...
  // INSTPAT("011100 ?????   ?????   ????? 00000 100000", clz    , N, fmt::print(ANSI_FMT("CLZ\n", ANSI_FG_BLUE));EXPT(isa_raise_intr(EC_RI, inst_state.pc)));
  INSTPAT("011100 ?????   ?????   ????? 00000 100000", clz    , R, inst_clz(src1, rd));
  INSTPAT("110011 ?????   ?????   ????? ????? ??????", pref   , N, );
  INSTPAT("010000 1????   ?????   ????? ????? 100000", wait   , N, );
