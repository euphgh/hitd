...
PaddrConfreg::PaddrConfreg(bool simulation, el::Logger *input_logger)
    : PaddrInterface(input_logger), output(input_logger) { /*{{{*/
  timer = 0;
...
bool PaddrConfreg::do_read(word_t addr, wen_t info, word_t *data) { /*{{{*/
  ...
  default:
    *(uint32_t *)data = 0;
    log_pt->warn(
        fmt::format("read  confreg not exist address: " HEX_WORD, addr));
    break;
  }
  return true;
} /*}}}*/

bool PaddrConfreg::do_write(word_t addr, wen_t info,
                            const word_t data) { /*{{{*/
  confreg_write++;
  ...
  default:
    log_pt->warn(
        fmt::format("write confreg not exist address: " HEX_WORD, addr));
    break;
  }
  return true;
} /*}}}*/
